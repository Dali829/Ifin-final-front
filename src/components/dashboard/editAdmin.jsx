...
const AddAdmin = () => {
    ...
    const fetchData = async () => {
        const response = await ServicesAdmin.getAdminByID(from);
    ...
    const handleClick = async (e) => {
        e.preventDefault();
        try {
            const response = await ServicesAdmin.updateAdmin(from, credentials);
            if (response.status === 201 || response.status === 200) {
                console.log("added");
                showToastMessage("Admin Edited")
    ...
    return (
        <div className="mx-16 mt-8">
            <div className="grid grid-cols-7 bg-[#0e5475]">
                <div className="col-span-5"><p className="text-white text-3xl m-2">Edit Admin</p>
                </div>
            ...
            <div className='grid grid-cols-5 text-sm text-[#0e5475]'>
                <div className='col-span-1 border border-[#0e5475] rounded-l-lg py-5 p-3 mt-4'>
                    Admin Name*
                    <input defaultValue={product.username} placeholder={product.username} onChange={(e) => credentials.username = e.target.value} className='border border-[#0e5475] rounded-lg p-1 ' />
                </div>
                ...
                <div className='col-span-1  border border-[#0e5475] py-5 p-3 mt-4'>
                    Email
                    <input type='text' defaultValue={product.email} placeholder={product.email} onChange={(e) => credentials.email = e.target.value} className='border border-[#0e5475] rounded-lg p-1 ' />

                </div>
                <div className='col-span-1  border border-[#0e5475] py-5 p-3 mt-4'>
                    Password
                    <input defaultValue={product.password} placeholder={product.password} type='text' onChange={(e) => credentials.password = e.target.value} className='border border-[#0e5475] rounded-lg p-1 ' />

                </div>
                <div className='col-span-1 rounded-r-lg border border-[#0e5475] py-5 p-3 mt-4'>
                    Super Admin<br />
                   {(String(product.superAdmin) == "true")?      <input checked type='checkbox' onChange={(e) => credentials.superAdmin = e.target.checked} className=' w-[30px] h-[30px] border border-[#0e5475] rounded-lg p-1 ' />:
